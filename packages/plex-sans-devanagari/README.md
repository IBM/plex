...
## Add the IBM Plex Sans Devanagari typeface to your device
...
```
npm install @ibm/plex-sans-devanagari
```
...
```scss
@use '@ibm/plex-sans-devanagari/scss' as PlexSansDevanagari with (
  $font-prefix: 'font-path'
);

@include PlexSansDevanagari.all();
```

### Font weights

We provide two ways of importing weights into your project. Using `PlexSansDevanagari.all()` will import all available weights and `PlexSansDevanagari.default()` will import only `light`, `regular` and `semibold` weight. You can also `disable` unwanted weights inside the `@use`. For example:
...
IBM Plex Sans Devanagari typeface weights map:

| Weight     | Enabled by default with | PlexSansDevanagari.all() | PlexSansDevanagari.default() |
| ---------- | ----------------------- | ------------------------ | ---------------------------- |
| bold       |                         | ✅                       | ✅                           |
| extralight |                         | ✅                       | ❌                           |
| light      |                         | ✅                       | ❌                           |
