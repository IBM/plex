...
## Add the IBM Plex Sans Chinese SC typeface to your device
...
```scss
@use '@ibm/plex-sans-sc/scss' as PlexSansSC with (
  $font-prefix: 'font-path'
);

@include PlexSansSC.all();
```

### Font weights

We provide two ways of importing weights into your project. Using `PlexSansSC.all()` will import all available weights and `PlexSansSC.default()` will import only `light`, `regular` and `semibold` weight. You can also `disable` unwanted weights inside the `@use`. For example:
...
IBM Plex Sans Chinese SC typeface weights map:

| Weight     | Enabled by default with | PlexSansSC.all() | PlexSansSC.default() |
| ---------- | ----------------------- | ---------------- | -------------------- |
| bold       |                         | ✅               | ✅                   |
| extralight |                         | ✅               | ❌                   |
| light      |                         | ✅               | ❌                   |
